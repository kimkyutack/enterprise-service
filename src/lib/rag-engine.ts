...
// 간단한 답변 생성 함수
const generateSimpleAnswer = (query: string, sources: any[]): string => {
...
// RAG 답변 생성 함수
export const generateAnswer = async (query: string): Promise<RAGResponse> => {
  try {
    // 1. 쿼리 임베딩 생성
    const queryEmbedding = await getEmbedding(query);

    // 2. 유사한 문서 검색
    const similarDocs = ragStore.searchSimilar(queryEmbedding, 5);
    ...
    // 3. 간단한 답변 생성
    const answer = generateSimpleAnswer(query, similarDocs);
...
export const addDocument = async (
  ...
  try {
    ...
    for (const chunk of chunks) {
      ...
      ragStore.addDocument(chunk, embedding, metadata);
    }
...
export const getDocumentCount = (): number => {
  return ragStore.getDocumentCount();
};

// 모든 문서 삭제 함수
export const clearDocuments = (): void => {
  ragStore.clearDocuments();
};

// RAG 엔진 객체 (기존 코드와의 호환성을 위해)
